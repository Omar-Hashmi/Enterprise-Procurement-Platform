...
/**
 * Service Layer — turns raw aggregates from analytics.repository.ts
...
class AnalyticsService {
  ...
  async getBudgetUtilization(fiscalYear: number) {
    return cacheService.getOrSet(
  ...
  async getDepartmentSpending(fiscalYear?: number) {
    return analyticsRepository.departmentSpending(fiscalYear);
  }

  async getProcurementSpendTrend(fiscalYear: number) {
      ...
      async () => {
        ...
        return {
          ...
          contracts: {
            ...
            compliancePercent: compliance.total === 0 ? 0 : Number(((compliance.verified / compliance.total) * 100).toFixed(2)),
          },
